<template>
  <section id="testQuestion">
    <b-card class="bg-light test-card rounded-0">
      <b-row align-h="end">
        <b-icon icon="files" class="mx-3 toolbar-btn" @click="duplicateQuestion"></b-icon>
        <b-icon icon="trash" class="mx-3 toolbar-btn" @click="deleteQuestion"></b-icon>
      </b-row>
      <b-row class="mt-3">
        <b-col cols="12">
          <b-form>
            <label class="d-inline-flex lbl-test-number"> {{ ordinal }}. </label>
            <b-form-input 
      ...
      <b-row class="mt-2">
        <b-col cols="6">
          <button class="btn-add-opt" @click="addOption">
            <b-icon class="plus-icon" icon="plus"></b-icon><span>Add Option</span>
          </button>
